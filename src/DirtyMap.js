...
// Extends the builtin JavaScript map.
export default class DirtyMap extends Map {
	...
	// to the dirty set.
	//
	// An optional $compareFunc may be passed, otherwise
	// `(a, b) => a === b` is used.
	//
	// Returns the dirty map for chaining.
	put(key, value, compareFunc = strictEquals) {
		checkCmpFunc(compareFunc)
		this._put(key, value, compareFunc)
		return this
	}
	...
	//
	// Returns the dirty map for chaining.
	putAll(obj, compareFunc = strictEquals) {
		checkCmpFunc(compareFunc)
		const keys = Object.getOwnPropertyNames(obj)

		for (const k of keys) {
			this._put(k, obj[k], compareFunc)
		}
	...
	//
	// Returns the dirty map for chaining.
	putMissing(key, value, compareFunc = strictEquals) {
		checkCmpFunc(compareFunc)
	...
	// An optional $compareFunc may be passed, otherwise
	// `(a, b) => a === b` is used.
	willPutDirty(key, value, compareFunc = strictEquals) {
		checkCmpFunc(compareFunc)

		return (
			this._dirty[key] || //
			!this.has(key) || //
			!this._isEqual(key, value, compareFunc)
		)
	}

	_put(k, v, cmpFunc) {
		if (!this._isEqual(k, v, cmpFunc)) {
			super.set(k, v)
			this._dirty.add(k)
		}
	}

	_isEqual(k, v, cmpFunc) {
		const hasKey = this.has(k)
		const currValue = this.get(k)
		const areEqual = cmpFunc ? cmpFunc : this._equals
